...
## Building and Running the OreSat Application

In order to build the OreSat application, or any other F´ application, we first need to generate a build directory. This can be done with the following commands:
...
The next step is to build the OreSat application's code.
```
fprime-util build
```
...
To run the ground system without starting the OreSat app:
```
...
```
cd OreSat/build-artifacts/<platform>/bin/
./OreSat -a 127.0.0.1 -p 50000
```
